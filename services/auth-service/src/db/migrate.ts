...
async function initMigrationsTable(): Promise<void> {
  ...
  // One-time migration: copy any existing rows from the shared table so a
  // running DB is not treated as fresh after this rename.
  await query(`
  ...
  `).catch(() => { /* schema_migrations may not exist on a fresh DB – that is fine */ });
}
